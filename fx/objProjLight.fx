...
float4x4 matMtl;	// Transformation matrix worldspace - lightspace
...
sampler2D sAttribute = sampler_state	//R= Specularity	G= Self-Illuminance		B= Reflectivity		A= Not Used
{
	texture = <mtlSkin3>;
};
samplerCUBE sTexture = sampler_state
{
	texture = <entSkin1>;
    MipFilter = Linear;
    MinFilter = Linear;
...
float4 ps_shader(out_vs In) : COLOR
{
	...
	float3 lightdir = normalize(pixdir);
	...
	float3 texpos = mul(100 * lightdir, (float3x3)matMtl);
	float4 Color = texCUBE(sTexture, normalize(texpos * vecSkill49.xyz));
	Color.a = 1;
