...
float4 vecSkill49;	// Direciton + Angle
float4 vecSkill53;	// X= Spotlight Falloff
...
float4 ps_shader(out_vs In) : COLOR
{
	...
	float3 lightdir = vecSkill49.xyz;
	float3 pixdir = Position - vecSkill41.xyz;
	
	float angle = dot(lightdir, normalize(pixdir));
	
	clip(angle - vecSkill49.w);
	
	float intensityCone = saturate(vecSkill53.x * (angle - vecSkill49.w));
	float intensityNormal = saturate(-vecSkill1.x * dot(Normal, normalize(pixdir)));

	float3 r = normalize(2 * dot(lightdir, Normal) * Normal - lightdir);
	intensityNormal += Attributes.r * vecSkill1.y * pow(max(dot(r, vecSkill5.xyz), 0), vecSkill1.z);
	
	float intensity = intensityCone * intensityNormal;
